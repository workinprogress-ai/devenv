...
Marks a task checkbox in an implementation plan as complete (`[x]`) or
incomplete (`[ ]`). Tasks are identified by their dotted number as they appear
in the plan (e.g. `2.3` or `1.4.2`).
...
- `TASK_NUMBER`: Dotted task number to update. Accepted formats: `X.Y` or `X.Y.Z`.
...
markdown-plan-complete-task 1.4.2 /path/to/Implementation_plan-001.md

# Undo — mark task 2.3 as incomplete
markdown-plan-complete-task --uncomplete 2.3
```

**Notes:**

- After updating the checkbox, the tool prints overall plan progress
