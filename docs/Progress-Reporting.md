...
How progress is tracked and reported in this workspace — the doctrine behind `/devenv-board`'s Answer class and the `Progress:` snapshot lines executor skills append to wrap-up comments.
...
## `/devenv-board` usage (Answer class)
...
**Report mode.** Triggered explicitly — "report on issues N, M, …", plural issue numbers in one ask, or `--report`. Compiles one markdown report to `.local-artifacts/` (ephemeral `tmpN.md`; posting to an issue requires explicit confirmation, and a multi-issue report defaults to not posting — no natural host issue). Structure: front matter with generation timestamp + resolved scope, then a fixed-shape section per issue (assigned engineer → status → progress block from the `plan-parse` summary plus census views (two invocations — multi-mode calls are rejected) → what's left → risks/drift with both readings → open questions). An epic in scope expands to an epic-level dual-view roll-up plus per-child sections — children resolved via the epic task list or roadmap issue links. An epic *without* report intent still gets the conversational dual view: an epic answers scope, never mode. All numbers are recomputed at generation time — reports are snapshots of a derived view, never stored progress.
...
- [Skills Catalog](./Skills.md) — the `/devenv-board` entry and its boundaries vs `/devenv-update-roadmap`, `/devenv-refine-plan`.
