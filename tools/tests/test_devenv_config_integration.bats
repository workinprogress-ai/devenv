...
@test "bootstrap.sh: has load_config function" {
    run grep -n "^load_config()" "$PROJECT_ROOT/.devcontainer/bootstrap.sh"
    [ "$status" -eq 0 ]
}

@test "bootstrap.sh: load_config called in default_tasks" {
    run grep "load_config" "$PROJECT_ROOT/.devcontainer/bootstrap.sh"
    [ "$status" -eq 0 ]
}

@test "bootstrap.sh: sources config-reader library" {
    run grep 'source.*config-reader.bash' "$PROJECT_ROOT/.devcontainer/bootstrap.sh"
    [ "$status" -eq 0 ]
}
