...
@test "actions module: loads, registers actions capability, idempotent" {
    provider_has_capability actions
    # shellcheck disable=SC1091
    source "$DEVENV_TOOLS/lib/providers/github/actions.bash"
    declare -F provider_actions_run_list >/dev/null
}

@test "run list: -R repo and flags pass through" {
    run provider_actions_run_list org/repo --branch main --limit 5
    assert_success
    grep -q "^gh run list -R org/repo --branch main --limit 5$" "$STUB_CALL_LOG"
}

@test "run view: run id and repo wired" {
    run provider_actions_run_view org/repo 12345
    assert_success
    grep -q "^gh run view 12345 -R org/repo$" "$STUB_CALL_LOG"
}

@test "run watch: polling stays domain-local with repo wiring" {
    run provider_actions_run_watch org/repo 42
    assert_success
    grep -q "^gh run watch 42 -R org/repo$" "$STUB_CALL_LOG"
}

@test "workflow list: passes repo" {
    run provider_actions_workflow_list org/repo
    assert_success
...
@test "run artifacts: REST endpoint wired with jq extraction" {
    ...
    run provider_actions_run_artifacts org/repo 77
    assert_success
...
@test "workflow run: dispatch wired with workflow name and repo" {
    run provider_actions_workflow_run org/repo build.yml --ref main
    assert_success
    grep -q "^gh workflow run build.yml -R org/repo --ref main$" "$STUB_CALL_LOG"
}

@test "run rerun: run id and repo wired" {
    run provider_actions_run_rerun org/repo 99
    assert_success
    grep -q "^gh run rerun 99 -R org/repo$" "$STUB_CALL_LOG"
}

@test "run cancel: -R form per inventory (github-helpers parity)" {
    run provider_actions_run_cancel org/repo 55
    assert_success
    grep -q "^gh run cancel -R org/repo 55$" "$STUB_CALL_LOG"
}

@test "run download: run id, repo, flags wired" {
    run provider_actions_run_download org/repo 77 --name art
    assert_success
...
@test "wait_for_branch: returns 0 immediately when no active runs" {
    ...
    export STUB_GH_API_RESPONSE="$TEST_TEMP_DIR/api.json"
    run provider_actions_wait_for_branch org/repo main 3
    assert_success
}
...
@test "wait_for_branch: query failure aborts with defined error" {
    ...
    stub_gh
    STUB_GH_FAIL=1 run provider_actions_wait_for_branch org/repo main 2
    assert_failure
