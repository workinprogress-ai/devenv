#!/usr/bin/env bash
# github/actions.bash - GitHub implementation of the actions domain facade.
#
# Implements provider_actions_* functions per tools/lib/providers/INVENTORY.md:
...
case " $PROVIDER_CAPABILITIES " in
    *" actions "*) ;;
    *) PROVIDER_CAPABILITIES="${PROVIDER_CAPABILITIES:+$PROVIDER_CAPABILITIES }actions" ;;
esac
...
# List workflow runs.
# Usage: provider_actions_run_list [repo] [--branch B] [--limit N] [FLAGS]
provider_actions_run_list() {
    local repo=""
    if [ $# -gt 0 ] && [[ "$1" != --* ]]; then
        repo="$1"; shift
    fi
...
# View a workflow run.
# Usage: provider_actions_run_view [repo] RUN_ID
provider_actions_run_view() {
    local repo=""
    ...
    local repo_args=()
    provider_gh_repo_args repo_args "$repo"
    gh run view "$1" "${repo_args[@]}"
}

# Watch a run to completion (polling belongs here, at the domain layer).
# Usage: provider_actions_run_watch [repo] RUN_ID
provider_actions_run_watch() {
    local repo=""
    if [ $# -gt 0 ] && [[ "$1" != --* && "$1" != ^[0-9]*$ ]]; then
        repo="$1"; shift
    fi
...
# List workflows.
# Usage: provider_actions_workflow_list [repo]
provider_actions_workflow_list() {
    local repo="$1"
    local repo_args=()
    provider_gh_repo_args repo_args "$repo"
...
# Trigger a workflow.
# Usage: provider_actions_workflow_run [repo] WORKFLOW [--ref REF] [FLAGS]
provider_actions_workflow_run() {
    local repo=""
    if [ $# -gt 0 ] && [[ "$1" != --* ]]; then
        repo="$1"; shift
    fi
...
# Usage: provider_actions_run_rerun [repo] RUN_ID
provider_actions_run_rerun() {
    local repo=""
    if [ $# -gt 0 ] && [[ "$1" != --* && "$1" != ^[0-9]*$ ]]; then
        repo="$1"; shift
    fi
...
# Cancel a run.
# Usage: provider_actions_run_cancel REPO RUN_ID
provider_actions_run_cancel() {
    local repo="$1" run_id="$2"
    gh run cancel -R "$repo" "$run_id"
}

# Download run artifacts.
# Usage: provider_actions_run_download [repo] RUN_ID [FLAGS]
provider_actions_run_download() {
    local repo=""
    if [ $# -gt 0 ] && [[ "$1" != --* && "$1" != ^[0-9]*$ ]]; then
        repo="$1"; shift
    fi
...
# ---------------------------------------------------------------------------
# Domain-local polling helper (github-helpers parity, facade-shaped)
# ---------------------------------------------------------------------------
...
# Usage: provider_actions_wait_for_branch REPO BRANCH [TIMEOUT_POLLS]
provider_actions_wait_for_branch() {
    ...
    for ((poll = 1; poll <= max_polls; poll++)); do
        active=$(gh run list -R "$repo" --branch "$branch" --limit 10 --json status --jq '[.[] | select(.status == "queued" or .status == "in_progress")] | length' 2>/dev/null) || {
            log_error "provider_actions_wait_for_branch: could not query runs for $repo@$branch"
            return 1
        }
        [ "${active:-0}" -eq 0 ] && return 0
        sleep 2
    done
    log_error "provider_actions_wait_for_branch: $repo@$branch did not settle within $max_polls polls"
    return 1
}
