...
appear in plans generated by devenv-create-plan.
...
given, the first Plan-*.md (or legacy Implementation_plan-*.md) found in the current directory is used.
...
Examples:
    ...
    $SCRIPT_NAME 1.1 1.2 /path/to/Plan-001.md
...
# the first Plan-*.md (or legacy Implementation_plan-*.md) in the current directory.
resolve_plan_file() {
    if [ -n "$PLAN_FILE" ]; then
        if [ ! -f "$PLAN_FILE" ]; then
    ...
    if [ -z "$found" ]; then
        log_error "No Plan-*.md (or legacy Implementation_plan-*.md) file found in the current directory."
