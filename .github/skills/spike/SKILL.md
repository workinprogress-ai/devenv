...
- A `/create-implementation-plan` invocation would stall on too many unknowns.
- A question can be answered faster by trying it than by reasoning about it.

If the user wants production code, use `/pair-programming` or `/delegation`. If the user wants to think out loud without producing artifacts, use `/rubber-duck`.
...
**Prototype**: `playground/spike-<topic>-<date>/` (if applicable)
...
<Anything the spike surfaced but didn't answer. These are inputs to the next spike or to `/create-implementation-plan`.>
```
...
- **Drifting into production code** — spikes are throwaway. If the prototype is becoming clean and complete, stop and write a plan with `/create-implementation-plan`. Resist the urge to "just polish it a bit".
...
- `/create-implementation-plan` — use spike findings as input to a real plan.
- `/rubber-duck` — lighter-weight thinking-out-loud without artifacts.
- `/pair-programming`, `/delegation` — for the actual implementation once the spike resolves.
