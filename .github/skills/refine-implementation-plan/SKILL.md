...
- A previous `/create-implementation-plan` run is now out of date.
- Discovery during Phase 1 revealed sub-tasks that didn't exist when the plan was written.

If there is no existing plan, stop and redirect to `/create-implementation-plan`.
...
- `/create-implementation-plan` — for brand-new plans from scratch.
- `/plan-status` — for reporting progress without modifying the plan.
- `/pair-programming` and `/delegation` — for actually executing the (refined) plan.
