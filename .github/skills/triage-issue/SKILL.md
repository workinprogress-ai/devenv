...
If the work is clear and you just want to do it, use `/pair-programming` or `/delegation`. If the issue is a complete spec ready to plan, use `/create-implementation-plan` or `/plan-from-spec`. If feasibility is unknown, use `/spike`. For a plain summary, the default agent's `summarize-github-issue-pr-notification` skill is faster.
...
- **XL** — uncertain enough to warrant `/spike` first.
...
- **Triaging the issue AND fixing it in the same run** — triage produces recommendations only. To implement, switch to `/pair-programming` or `/delegation`.
- **Over-explaining classifications** — one-line reasoning per field. Long justifications are noise.

## Sibling skills

- `/pair-programming`, `/delegation` — once triaged, to implement.
- `/create-implementation-plan`, `/plan-from-spec` — when the issue is complete and ready to plan.
- `/spike` — when the issue's feasibility or approach is unknown (size = XL).
