...
- Tasks follow the **condensed step format** — see [task-format.md](../devenv-create-implementation-plan/references/task-format.md): `- [ ] **N.M [S|M|L] Title** ([additional context](#anchor))` header as the primary execution step, with optional `Files:` / `decision:` / `owner:` / `depends on` metadata only when it improves clarity. Every task gets an `[S|M|L]` size label. Do not generate vague title-only tasks.
