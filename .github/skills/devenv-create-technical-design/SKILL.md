...
Track open questions as `Q-NNN` items. See [Q-NNN format](../_conventions.md#open-questions-log-q-nnn). Every Q-NNN must reach `resolved` or `deferred` before writing the output.
...
See [architecture-doc-structure.md](./references/architecture-doc-structure.md) for the required sections and content guide for `Architecture_and_implementation.md`.
