...
If the user wants code, use `/pair-programming` or `/delegation`. If the user wants written output (a plan, a findings doc, a review), use the appropriate skill for that. If the question is factual ("how does X work in this codebase?"), the default agent is faster; for unknowns that need an experiment, use `/spike`.
...
- **Does not write or modify code.** Even if the user asks "what would the code look like?", redirect: "Want to switch to `/pair-programming` for that?"
...
2. **Suggest a next-step skill** if one fits naturally:
   - The user has clarity and a path forward → `/create-implementation-plan` or `/plan-from-spec`.
   - The user has an unverified assumption that blocks progress → `/spike`.
...
- `/spike` — when the question needs an experiment, not a conversation.
- `/create-implementation-plan` — when the user gains clarity and wants to plan the work.
- `/plan-from-spec` — when the conversation produced enough structure to draft from.
- `/pair-programming`, `/delegation` — for the actual implementation once the thinking resolves.
