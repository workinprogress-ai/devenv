...
This skill is strictly read-only. It does not write to the plan, post issue comments, or update the issue body. If the user asks for changes, redirect to `/refine-implementation-plan`.
...
- **Modifying the plan** — even a "small fix" while reading is out of scope. Use `/refine-implementation-plan`.

## Sibling skills

- `/create-implementation-plan` — for brand-new plans.
- `/refine-implementation-plan` — for revising plans after discovery.
- `/pair-programming` and `/delegation` — for executing plan tasks.
