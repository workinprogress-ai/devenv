...
`/devenv-groom` standardizes how we choose among the remaining design paths.
...
- Coding execution -> [`/devenv-pair`](../devenv-pair/SKILL.md), or [`/devenv-delegate`](../devenv-delegate/SKILL.md) when the user commissions an autonomous mechanical run
...
This pass is a checklist item, not a redesign loop — a few minutes per artifact at most. If it starts arguing with the whole blueprint, that is the existing escalation to `/devenv-design` (or an upstream-impact issue when the artifact itself is wrong), not this gate.

## Routing guardrails

- If the user primarily needs alternatives/trade-offs -> route to `/devenv-design`.
...
- **Precedence rule:** if an `Plan-*.md` is in-flight and the user's goal is to unblock active implementation, stay in `/devenv-groom` by default and facilitate decision closure here.
- Route to `/devenv-design` when two or more viable approaches are still live and the team needs an explicit recommendation.
- Route to `/devenv-design` for a single large blocker/question when it needs deeper option-weighing, but the expected outcome is still a bounded plan change rather than a broader design reset.
...
- Use findings to either close decisions safely in grooming, route one bounded blocker to `/devenv-design`, or route broader drift to upstream design reshaping.
...
- **No plan exists yet for the groomed scope** → [`/devenv-plan`](../devenv-plan/SKILL.md). This is the default after initial grooming of new scope. If the attack plan has multiple rows, create the child issues first, then plan one slice at a time.
...
- downstream mapping for the plan skill (`/devenv-refine-plan` for an existing plan, `/devenv-plan` when no plan exists yet) (`Watch Outs / Decisions`, task `decision:` metadata, `## Pending Questions`, `## Appendix`),
...
- If invoked from `/devenv-pair` or `/devenv-delegate` handoff context, preserve escalation evidence and classification context.
...
(Use `/devenv-refine-plan` when a plan already exists; `/devenv-plan` when none does — the same mapping feeds both.)
...
Per the shared [next-step offer](../common/references/execution-gates.md#next-step-offer-wrap-up-convention) convention, wrap-up ends with one structured offer: the grooming artifact hands to `/devenv-plan` ("plan this") for the selected slice, or `/devenv-create-roadmap` if the attack plan spans multiple deliverables.
...
- Recommending `/devenv-refine-plan` when no plan exists for the groomed scope — plan generation from a fresh grooming artifact starts at `/devenv-plan`; refine is only for aligning an already-existing plan.
...
- [`/devenv-design`](../devenv-design/SKILL.md) — approach-level, pre-requirement questions (component shape, cross-cutting concerns); groom owns **bounded-requirement design**: a fleshed-out requirement whose ambiguity needs resolving within a change-bound piece of work
- [`/devenv-plan`](../devenv-plan/SKILL.md) — downstream when no plan exists for the groomed scope
