...
**Referenced-material rule.** When a question references material — acceptance criteria, phase outlines, trade-off option sets, diffs, findings, report excerpts — present that material in chat, fully formatted and labeled with stable IDs, in the same turn immediately before invoking the question tool. The questions then reference the labels ("Which of AC-2/AC-4 above should change?"), never re-embed the material into the question fields. Use rendered markdown by default; use fenced blocks only for exact-format content (command output, diffs, code, alignment-sensitive layouts). With multiple questions, present all referenced material first, ordered to match the questions — or split into sequential material-plus-question invocations when the combined material exceeds roughly a screen. Self-contained questions whose options carry their own context skip the preamble; do not manufacture material. Each question whose material lives in chat carries a one-line pointer in its `message` field (e.g. "See the AC list above — AC-1..AC-5 marked *(inferred)*").
