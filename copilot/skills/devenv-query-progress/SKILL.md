...
- "report on issues 1, 5, 9" / plural issue numbers passed in one ask → report mode (see Report mode below)
...
- **Reports on request:** markdown to `.local-artifacts/tmpN.md` (ephemeral, next free number — see [ephemeral markdown conventions](../_conventions.md#ephemeral-markdown-files-tmpnmd)), or posted as an issue comment with explicit confirmation.

### Report mode

Explicit trigger only — one of: the `report` keyword ("report on issues 1, 5, 9"), plural issue numbers in a single ask, or `--report` in the argument list. **Scope is orthogonal to mode:** an epic number answers *what to investigate*, never *how to present* — an epic without report intent gets today's conversational dual view unchanged; an epic inside a report scope expands as below.
...
Read-only guarantees are unchanged — report mode adds no write surface.
