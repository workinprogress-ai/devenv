...
When invoked via `/devenv-audit`, follow the protocol below exactly.
...
Do **not** use for reviewing a single PR (use `/devenv-review`), for pair programming on specific tasks (use `/devenv-pair`), or for breaking down findings into actionable tasks (use `/devenv-plan` once the audit is in hand).
