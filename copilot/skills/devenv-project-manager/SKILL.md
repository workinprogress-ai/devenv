...
Do **not** use for: plan stewardship (executor/refine skills), single-issue intake triage (`/devenv-triage-issue`), grooming execution (`/devenv-grooming`), plan creation (`/devenv-create-plan`), roadmap status sync (`/devenv-update-roadmap`), or in-run status for the current pair/delegation session (those skills answer their own runs).
...
6. **Label hygiene** — labels outside the configured vocabulary (`tools/config/issues-config.yml` + org labels), inconsistent type labels, missing labels the org's conventions require.
...
- **Grooming candidates**: `To-Groom`/`TBD` past threshold, parentless `Ready` items, design-stale items — each with a one-line why; route → `/devenv-grooming`.
...
- **Triage candidates**: unprojected/unlabeled new issues; route → `/devenv-triage-issue`.
...
- `/devenv-triage-issue` — single-issue intake routing; this skill recommends, that skill triages.
- `/devenv-grooming` — grooming execution; this skill nominates candidates.
- `/devenv-create-plan` — plan creation; this skill routes plan-sized work there.
