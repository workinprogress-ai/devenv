...
Write `spike-NNN-<topic>.md` under the target repo's `.local-artifacts/` folder (the [standard local markdown folder](../_conventions.md#standard-local-markdown-folder-local-artifacts); use the workspace's `.local-artifacts/` when no single target repo is active), where `NNN` comes from `next-id --pattern 'spike-{N}-*' --width 3 --dir <artifacts-folder> --filename` (never overwrites an existing spike doc). Structure:
