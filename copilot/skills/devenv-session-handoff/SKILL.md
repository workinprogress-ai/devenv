...
- "Update [Plan-X.md](Plan-X.md) with progress? (y/n)" — hands off to `/devenv-refine-plan` (surgical mode for ticks/notes, revision mode for structural changes).
- `/devenv-project-manager` — for live progress and board questions ("how far along is this?", "what's in flight?"). That skill derives read-only answers from plans/issues at query time; this skill writes a narrative handoff for the next contributor — handoff context, not a progress report.
