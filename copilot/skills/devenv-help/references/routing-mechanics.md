...
- Broad/class/focus-area bug hunting (no single known bug) → `/devenv-tech-debt-audit`.
- One specific known bug to root-cause or fix → `/devenv-bug-hunter` (diagnose mode).
- Suspected but unconfirmed bug (specific observation + expected behavior) → `/devenv-bug-hunter` (verify mode).
- A Critical/High correctness risk from `/devenv-tech-debt-audit` is unconfirmed until hunted → `/devenv-bug-hunter` (verify mode); on FOUND the same skill continues to diagnose and fix.
...
comments, no writes — full metadata triage stays with `/devenv-triage-issue`.

1. Fetch the issue via `issue-get <N>` (title, body, labels).
2. Classify the issue shape using the workflow routing table in
   `/devenv-triage-issue` — it is the canonical issue-shape → skill mapping.
...
Route to `/devenv-triage-issue` instead when the user clearly wants the full
...
> - Weigh options and get a recommendation first → `/devenv-design-discussion`
...
Routing: task-by-task / high-impact → `/devenv-pair-programming`; commissioned
autonomous mechanical run → `/devenv-delegation` (never route casual "do it"
...
- **Feature discovery vs delivery** — adding a feature to an existing component
  while still deciding the approach → `/devenv-design-discussion` first;
  approach already chosen → Plan/Build.
- **Architecture intake** — default component-level architecture intake to
  `/devenv-grooming` unless the user explicitly requests one specialized design
  skill. Alternatives/trade-offs wanted → `/devenv-design-discussion`. "The
  current approach is wrong" → `/devenv-grooming`, not refine. One bounded
  blocker → `/devenv-design-discussion`; accumulating questions / sweeping
  redesign → `/devenv-grooming`.
- **Upstream artifacts** — planning from a design/spike artifact with no
  grooming artifact → `/devenv-grooming` first unless explicitly bypassed.
...
- **Direct-plan exception** — user explicitly wants a plan without grooming
  (thin-air context, pasted notes, concrete spec) → `/devenv-create-plan`.
  Plan too large/risky for one issue → `/devenv-grooming` for
  Feature/Fix/Task redivision, then back to `/devenv-create-plan` for one slice.
- **Build** — no high-impact phases to `/devenv-delegation`; delegation needs an
  explicit commission — actively collaborating means `/devenv-pair-programming`
...
- **Escalation mid-execution** — small local plan adjustment → stay in the
  ...
  blocker → `/devenv-design-discussion`; accumulated architecture issues →
  `/devenv-grooming`.
- **Status queries** — "how is X going?" across plans/issues, board hygiene,
  what's blocking → `/devenv-project-manager` (executor skills answer only
...
- Implement now → no plan → `/devenv-create-plan`; plan + high-impact →
  `/devenv-pair-programming`; plan + mechanical + commissioned autonomous run →
  `/devenv-delegation`.
- Decide direction first → weigh alternatives → `/devenv-design-discussion`;
  component-level intake → `/devenv-grooming`; specialized design skills only
  on explicit request.
