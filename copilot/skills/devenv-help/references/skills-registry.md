...
| `/devenv-bug-hunter` | End-to-end bug skill, three entry modes: verify (aggressive hypothesis hunt — oracle, discriminating  tests/instrumentation, FOUND / NOT-FOUND / INCONCLUSIVE verdict), diagnose (read-only call-chain trace to root cause for an established bug, findings report with mandatory effort estimate, capture to local artifact, and issue sync — existing / create / local-only), fix (sized resolution menu: kill-now strike for small fixes, plan handoff preferred for larger bugs, pair/delegation handoff, or self-fix; test-first, per-change confirm, never commits). One invocation carries verify→diagnose→fix | "is this a bug?", "I keep seeing X but expected Y", "go hunting", "fix this bug", "find the root cause", "why is X broken", "diagnose this", GH issue # with a bug | broad suspicion-less survey → `/devenv-tech-debt-audit`; feature work → `/devenv-create-plan`; general code Q&A → `/devenv-chat-with-code`; feasibility research → `/devenv-spike` |
...
For epic-scale work where specifications need to translate into architecture and a sequenced delivery plan with GitHub issues. Each roadmap step then spawns a technical design (for new components) and a plan as work begins.
