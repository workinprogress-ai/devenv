...
| `/devenv-bug-hunter` | End-to-end bug skill, three entry modes: verify (aggressive hypothesis hunt — oracle, discriminating tests/instrumentation, FOUND / NOT-FOUND / INCONCLUSIVE verdict), diagnose (read-only call-chain trace to root cause for an established bug, findings report with confidence + fix sequence), fix (test-first, per-change confirm, never commits; user picks plan / fix-now / self-fix). One invocation carries verify→diagnose→fix | "is this a bug?", "I keep seeing X but expected Y", "go hunting", "fix this bug", "find the root cause", "why is X broken", "diagnose this", an issue number (#N) describing a bug | broad suspicion-less survey → `/devenv-tech-debt-audit`; feature work → `/devenv-create-plan`; general code Q&A → `/devenv-chat-with-code`; feasibility research → `/devenv-spike` |
| `/devenv-triage-issue` | Triage an issue on two layers — workflow routing (which skill should handle it: bug / plan / groom / blueprint+roadmap / spike / audit / docs / specifications / refine-queue) and GitHub metadata (labels, priority, size, duplicates, clarifying comment) | "triage this issue", "what should handle this issue", "route this issue", "triage #123", "label and size this", "is this a duplicate" | implementing the routed work → the routed skill; plain summaries → default agent |
...
| Skill | One-line purpose | USE WHEN triggers | NOT FOR |
| --- | --- | --- | --- |
| `/devenv-pair-programming` | Collaborative implementation with bounded autonomy span — one task or small chunk per human touchpoint; the engineer stays in control, and the AI keeps the plan aligned to actual work while raised decision gates block all mutating actions until explicitly resolved. High-impact work goes here at any size | "pair program", "let's pair on this", "work on this issue with me", "implement this together", plus any build work the user wants to stay close to, task by task | long unattended runs of mechanical work → `/devenv-delegation` (explicit invocation required); pure exploration → `/devenv-spike` |
| `/devenv-delegation` | Commissioned autonomous run for phase-scale mechanical work — entered only by explicit `/devenv-delegation` invocation; assistant-led execution with user review, phase task-list refresh as the execution ledger, and raised decision gates blocking further mutation until explicitly resolved. Accepts ad-hoc task lists (viability audit + materialization into a plan file) and bare issues (issue-backed input; issue intake gate materializes a small plan first) — never in-context lists | "delegate this to you", "you take this", "run with this", "implement this plan", "do this for me", "take issue #N" | high-impact phases (public API, data shape, security, novel architecture) → `/devenv-pair-programming`; work without a plan → create one first; granular task-by-task collaboration → `/devenv-pair-programming` |
...
/devenv-triage-issue
  → /devenv-create-plan
    ...
    → /devenv-delegation            (commissioned autonomous mechanical run)
    → /devenv-pre-commit
    → /devenv-open-pr
      → /devenv-address-pr-comments
        → /devenv-pre-commit
...
**Start here:** `/devenv-triage-issue` (or `/devenv-create-plan` if already triaged)
    ...
    → /devenv-create-roadmap         (creates issues across component repos)
      → /devenv-grooming             (classify component-level design work before tasks are written)
        → /devenv-design-discussion  (if approaches need weighing)
          → /devenv-create-plan   (per roadmap step, draws from the current design delta)
...
/devenv-create-blueprint             (or existing blueprint as input)
  → /devenv-grooming                   (confirm whether component-level design work is needed)
    → /devenv-design-discussion        (if approaches need weighing)
      → /devenv-create-plan   (task breakdown draws from the current design)
        → /devenv-pair-programming / /devenv-delegation (commissioned autonomous mechanical run)
...
/devenv-document                     (understand and write up the existing system)
  ...
  → /devenv-create-plan   (if a specific deliverable is already defined)
```
...
/devenv-design-discussion or /devenv-spike
  → /devenv-grooming                     (capture design delta + issue attack plan)
    → /devenv-create-plan   (one selected issue slice)
      → /devenv-pair-programming / /devenv-delegation (commissioned autonomous mechanical run)
```
...
- If the user explicitly chooses to skip grooming and provides sufficient context, start with `/devenv-create-plan`.
...
```
/devenv-pair-programming or /devenv-delegation (commissioned autonomous mechanical run for either)
