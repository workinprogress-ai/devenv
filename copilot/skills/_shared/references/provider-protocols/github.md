...
> **Fork note:** this document defines GitHub transport for devenv's skill
> layer. A fork adapting to another provider replaces this file (and the
> `tools/lib/providers/<provider>/` modules); skill bodies never change.
> Activation key: `devenv.config` `[provider] name` (see
> `tools/lib/providers/provider-core.bash`).

All external operations go through the workspace wrapper commands below —
**never the raw provider CLI** (`gh`). Operations no wrapper covers are
surfaced as tooling gaps, not worked around with raw CLI calls.
...
- **Never use GitKraken MCP tools** (`mcp_gitkraken_*`) — the workspace does
  not use GitKraken.
- **Never embed tokens** in remote URLs, env exports, or command arguments.
...
Used when execution reveals an upstream design artifact is wrong; body covers
what was discovered, why it matters, and the affected upstream sections.
