...
- Work without an existing plan → use [`/devenv-plan`](../devenv-plan/SKILL.md) first.
- High-impact / collaborative work → use [`/devenv-pair`](../devenv-pair/SKILL.md).
...
Follow pair-programming's canonical [Handling Unexpected Bug Discoveries](../devenv-pair/SKILL.md#handling-unexpected-bug-discoveries) protocol — it applies identically here, including the four-case scope classification and the test-integrity guardrail. Delegation carries no local delta: the cases, responses, and core rule are exactly as defined there.
...
- **No plan or too thin**: refuse delegation. Redirect to `/devenv-plan` to draft one first, or `/devenv-refine-plan` if the plan exists but lacks the human-facing sections. Exception: a genuinely micro commission (below) materializes a stub ledger instead of refusing.
...
4. **Route on fail.** If any audit condition fails, refuse the list and recommend `/devenv-plan` — audit failure is itself evidence the work needs real planning, not transcription.
...
- If any in-scope phase is `better-as-pair`, flag it clearly: explain the risk and recommend switching to `/devenv-pair` for that phase. Then wait for the user's response. If the user wants to proceed with delegation anyway, accept it — note the concern in that phase's completion handback and proceed.
- If **all** in-scope phases are `better-as-pair`, recommend switching to `/devenv-pair` entirely. If the user declines and wants to continue with delegation, accept that and proceed.
...
- **Foreign working-tree changes.** Working-tree edits outside the run's own tracked changes — whoever wrote them: the user in an editor, **another commissioned session** (a second `/devenv-delegate` or a `/devenv-pair` run is just as much a foreign writer as a human), or anything else. The run's files are presumed exclusively its own relative to the kickoff provenance baseline while the user is away, so fresh foreign edits (especially near the plan's `Files:` sets) risk clobbering concurrent work. Pause and surface; checkpoint mode defers the surface to the next boundary unless the changes collide with files the run has already edited. **Plan-file edits the run didn't make are always this trigger** (single-writer rule, kickoff section). **Staging and commits are never this trigger:** the user may be reviewing in real time, and files moving into the index or into commits (including the run's own outputs) are normal review activity — note them in the handback's working-tree provenance line and continue.
...
- A task turns out to be high-impact mid-implementation — suggest switching to `/devenv-pair` for the remainder, or pausing so the user can redirect.
...
> **Switching to pair-programming mid-session:** if the user wants to switch, tell them explicitly: *"To get the full pair-programming rules, please start a new chat and invoke `/devenv-pair` — continuing in this session means the pair-programming skill isn't loaded and its rules won't apply."* Do not continue in delegation mode pretending to pair-program.
...
- Treating unattended execution as legitimate without an explicit `/devenv-delegate` commission — this skill's rules apply only because the user invoked it; they do not transfer to sessions running under other skills.
...
- Attempting a mutating git operation as a repair — unstaging, resetting to the last commit, restoring files from HEAD after corrupting them — instead of stopping immediately and handing the recovery to the user — including under guard-shaped framing (an `echo` chained after the command with `;` does not gate it).
