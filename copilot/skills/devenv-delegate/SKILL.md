...
- **Foreign working-tree changes.** Working-tree edits outside the run's own tracked changes — the run's files are presumed exclusively its own while the user is away, so fresh foreign edits (especially near the plan's `Files:` sets) risk clobbering concurrent user work. Pause and surface; checkpoint mode defers the surface to the next boundary unless the changes collide with files the run has already edited. **Staging and commits are never this trigger:** the user may be reviewing in real time, and files moving into the index or into commits (including the run's own outputs) are normal review activity — note them in the handback's working-tree provenance line and continue.
