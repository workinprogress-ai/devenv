...
| `/devenv-query-progress` | Read-only progress reporting across plans, grooming issues, and roadmaps — derives metrics from checkboxes + issue state via `plan-parse --summary`, rolls up the issue tree (direct + descendant plans), reports risk callouts, trends from `Progress:` snapshot lines, and git-derived signals; never writes anything. Report mode: "report on issues N, M, …" / plural issues / `--report` compiles a markdown report (epic scope expands to child sections) to `.local-artifacts/` | "how is X going?", "what's the progress on issue N / this plan?", "what's left on the audit work?", "are we on track?", "what's blocking the epic?", "report on issues …", cross-plan/issue status roll-ups | updating anything → executor skills or `/devenv-refine-plan`; syncing roadmap status → `/devenv-update-roadmap`; plan revision → `/devenv-refine-plan`; session summaries → `/devenv-session-handoff`; code review → `/devenv-code-review`; in-run status for the current pair/delegation session → those skills |
