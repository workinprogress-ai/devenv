...
| Skill | One-line purpose | USE WHEN triggers | NOT FOR |
| --- | --- | --- | --- |
| `/devenv-pair-programming` | Collaborative implementation — the engineer stays in control, and the AI keeps the plan aligned to actual work while raised decision gates block all mutating actions until explicitly resolved | "pair program", "let's pair on this", "work on this issue with me", "implement this together" | solo "do this for me" tasks → `/devenv-delegation`; pure exploration → `/devenv-spike` |
| `/devenv-delegation` | Delegated implementation support — assistant-led execution with user review, using phase task-list refresh as the execution ledger and keeping the plan aligned to actual work while raised decision gates block further mutation until explicitly resolved | "delegate this to you", "you take this", "run with this", "implement this plan", "do this for me" | high-impact phases (public API, data shape, security, novel architecture) → `/devenv-pair-programming`; work without a plan → create one first |
