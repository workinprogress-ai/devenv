...
| `/devenv-tech-debt-audit` | Opinionated codebase audit that surfaces tech debt and correctness/bug risks, optionally focused by module or bug class | "hunt for bugs", "find bug risks", "look for race conditions", "audit this area for bugs", "bug hunt in <module>", "scan for null/date/idempotency bugs" | single known bug root-cause/fix → `/devenv-bug-fix`; verifying one suspected (unconfirmed) bug → `/devenv-bug-hunter`; single PR review → `/devenv-code-review`; collaborative implementation → `/devenv-pair-programming` |
| `/devenv-bug-hunter` | Aggressive verification of a suspected bug: formalizes observation-vs-expectation oracle, enumerates hypotheses, eliminates with discriminating tests/instrumentation, delivers FOUND / NOT-FOUND / INCONCLUSIVE verdict with hunt report; hands off to bug-fix on FOUND | "is this a bug?", "something looks off — can you verify?", "I keep seeing X but expected Y", "unleash the bug hunter", "go hunting", "hunt this down" | confirmed bug needing root-cause fix → `/devenv-bug-fix`; broad suspicion-less survey → `/devenv-tech-debt-audit`; feasibility research → `/devenv-spike` |
| `/devenv-bug-fix` | Investigate a bug from a GH issue or description, trace root cause through the codebase, produce findings report with proposed fix — user then chooses: create a plan, fix now, or fix themselves | "fix this bug", "investigate this issue", "find the root cause", "why is X broken", "diagnose this", GH issue # with a bug report | feature work → `/devenv-create-implementation-plan`; general codebase Q&A → `/devenv-chat-with-code`; verifying an unconfirmed suspicion aggressively → `/devenv-bug-hunter`; feasibility research → `/devenv-spike` |
