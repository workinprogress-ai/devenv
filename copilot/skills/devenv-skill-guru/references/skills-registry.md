...
| `/devenv-project-manager` | On-demand project management for the issue landscape — project members and unprojected tickets alike. Four classes: Answer (read-only status/composition/progress — board status, epic rollups via workflow-core derivation, drill-downs, return digests, personal work queues, cross-repo portfolio views), Assess (hygiene sweeps: membership gaps, status drift, staleness from config thresholds, orphan + label hygiene — findings with evidence), Recommend (grooming/flow/triage candidates, routed — never executed here), Act (writes only on explicit instruction or consented batch; workflow-engine validation governs, never bypassed). Progress stays a derived view via `plan-parse --summary`; report mode compiles markdown to `.local-artifacts/` on explicit trigger | "project status", "board status", "what's in flight", "how is the epic going?", "clean up the board", "board hygiene", "what's drifted?", "what's stale?", "what should be groomed next?", "what should I look at?", "what changed while I was away?", "what's unprojected?", "move #N to X", cross-plan/issue status roll-ups | plan stewardship → executor skills or `/devenv-refine-plan`; grooming execution → `/devenv-grooming`; single-issue intake triage → `/devenv-triage-issue`; plan creation → `/devenv-create-plan`; roadmap status sync → `/devenv-update-roadmap`; session summaries → `/devenv-session-handoff`; in-run status for the current pair/delegation session → those skills |
