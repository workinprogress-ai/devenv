...
3. **Apply stage-specific guardrails before finalizing:**
   ...
   - Status-query guardrail: for "how is X going?" / progress / on-track / what's-blocking questions across plans or issues, route to `/devenv-query-progress` (read-only derived reporting) — not to the executor skills, which only answer their own in-run status.
