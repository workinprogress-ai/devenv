---
name: devenv-triage
description: Triage issues on two layers — workflow routing (which skill should handle it: bug → /devenv-hunt, ready-to-plan → /devenv-plan, design-unclear → /devenv-groom, one bounded question → /devenv-design, cross-component epic → /devenv-create-blueprint + /devenv-create-roadmap, unknown feasibility → /devenv-research, debt assessment → /devenv-audit, docs gap → /devenv-document, missing functional definition → /devenv-write-specifications, upstream-impact label → refine skills) and GitHub metadata (type, labels, priority, size, duplicates, clarifying comment). USE WHEN the user says "triage this issue", "what should handle this issue", "route this issue", "triage #123", "label and size this", "is this a duplicate", or hands off a fresh untriaged issue / batch / pasted issue text. Auto-detects input: issue number(s) → fetched via issue-get; pasted text → triaged in place. Bundles all proposed writes into a single y/n confirm before applying. DO NOT USE FOR implementing the issue (skills routed to take over) or plain summaries (use the default agent / summarize-github-issue-pr-notification).
...
> **Boundary:** `/devenv-help` (issue mode) covers the lightweight end of this — read-only fetch + which-skill-handles-this recommendation, no metadata. If the user only wants a quick routing answer, either skill works; anything involving labels, type, priority, size, duplicates, or batch processing belongs here.
...
|---|---|---|
| Describes broken behavior, suspected or confirmed | `/devenv-hunt` | The bug pipeline: verify (existence uncertain) → diagnose (existence established, root cause unknown) → fix — one invocation carries the whole pipeline |
| Single-component work, approach already chosen, issue is complete | `/devenv-plan` | Ready to plan: phases and tasks |
| Single-component work, approach unclear | `/devenv-groom` | Component-level design direction must settle before tasks can be written |
| One focused design question / blocker | `/devenv-design` | Bounded option-weighing, not full planning |
...
| Feasibility or approach unknown, needs research | `/devenv-research` | Throwaway investigation before any planning makes sense |
| Codebase health / debt / architecture assessment | `/devenv-audit` | Audit produces findings + issue creation, not implementation |
...
- **XL** — uncertain enough to warrant `/devenv-research` first.
...
- `/devenv-hunt` — the bug pipeline (verify / diagnose / fix).
- `/devenv-plan` — complete single-component issues ready to plan.
- `/devenv-groom` — component design direction before planning.
- `/devenv-design` — one bounded design question.
...
- `/devenv-research` — unknown feasibility or approach.
- `/devenv-audit` — codebase health assessment.
