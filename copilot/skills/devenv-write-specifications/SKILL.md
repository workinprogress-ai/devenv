...
Trigger phrases:
- "write specifications" / "specifications document" / "define the specifications for X"
- "what should the system do?" / "write specifications"
- "interview me for specifications" / "help me capture specifications"
- A system idea is handed off before any planning has begun

### Continuing: brainstorming and exploring changes to existing doc

If you have an existing `Specifications-*.md` and want to explore new ideas, brainstorm implications, or integrate new input **before committing changes**, pass the file path as the argument. This mode treats the session as mid-gathering — a natural continuation where new thoughts have emerged. You'll:
- Brainstorm implications: "What would change if we added this? What would break?"
- Explore ripple effects across the existing specification set
- Run consistency checks against the existing doc
- Integrate new specifications or revise existing ones
...
- "I have more specifications" (with existing doc path)
...
- **Think like a user, not an engineer.** Specifications describe behaviors and outcomes, not architectures or technologies.
- **Concrete beats abstract.** Every specification item should be specific enough that two independent teams would build recognizably similar things from it.
- **Completeness over speed.** Missing specification items cost far more to discover in implementation than to find here. Probe for gaps, edge cases, and the unhappy path.
...
Specifications authoring can span multiple sessions, and brainstorming conversations benefit from continuity. Maintain a `session_memory-specifications.md` file in the **target repo root** to preserve state across sessions.
...
- Gaps identified in the vision or specifications
...
**When the specifications are complete and approved**: offer to delete it. Do not merge it to the main branch.
...
See [specifications-template.md](./references/specifications-template.md) for the full document structure.

Do not write the file until Phase 3 is approved. During the session, work in chat and update `session_memory-specifications.md`.

### Multi-document projects (one doc per epic)

A single doc is the default. For large initiatives (multiple distinct epics, 30+ specification items, or distinct stakeholder groups), split into one `Specifications-<epic>-NNN.md` per epic with a category-prefix ID scheme (e.g. `ORD-NNN`). Produce an `Index.md` as the canonical entry point. Cross-doc dependencies use `Depends on: AUTH-003 (Specifications-auth-001.md)`.
...
This is a three-phase process for fresh specifications authoring. **Stop at each checkpoint** and wait for explicit approval before proceeding.

### Continuation mode (existing doc): brainstorming-first

If the argument is a path to an existing `Specifications-*.md`, enter **continuation mode** — a natural mid-gathering brainstorm where new ideas have emerged.
...
4. **Run the brainstorm.** As the user describes the new thought:
   - Ask probing questions about implications: *"Would that affect SPEC-004 and SPEC-012? How would [actor] experience that?"*
   - Explicitly surface ripple effects: *"If we do that, SPEC-008's assumption about [X] breaks. What would that mean?"*
   - Identify tensions early and call them out: *"That conflicts with [vision section / SPEC-XXX / GROUP-YY]. How do you want to resolve that?"*
   ...
   - Check whether the vision itself needs to shift, not just individual specification items

5. **Explore implications systematically.** Don't jump straight to "what new specifications do we need": first work through the tension and see what actually changes. This often surfaces:
   - Specification Items that become impossible or need revision
...
6. **Update as you go** (in session memory, not the file). Track:
   ...
   - Which existing specifications are affected
...
**Ask about existing communications** (transcripts, email threads, recordings, etc.) before starting cold. Dispatch the `Explore` subagent per artifact to produce structured summaries (see [Explore subagent dispatch](../_conventions.md#explore-subagent-dispatch)); surface each summary for validation before extracting specification material. Cite the source in `session_memory-specifications.md` for any specification item that traces back to a communication.
...
Scenarios are one of the most powerful tools in specifications authoring — they help stakeholders visualise the system in action and surface specification items that abstract descriptions miss. Aim for at least 3–5 concrete narrative walkthroughs; don't cap them artificially. The right number is however many it takes to cover the meaningful situations.
...
**Scenario → specification item backlinking (done in Phase 2):** Once Phase 2 specification items are written, come back to each scenario and add inline markdown links to the specifications it illustrates. Note in session memory that backlinking is pending for each scenario.
...
### Phase 2: Specification Items

**Goal:** Break the vision into specific, acceptance-criteria-bearing, dependency-ordered specification items.

#### Step 1: Agree on ID prefix scheme

For smaller systems with a single specifications doc, `SPEC-NNN` is sufficient.

For larger systems with distinct functional areas in **one doc**, use category prefixes (e.g. `AUTH-001`, `ORD-003`).

For **multi-document projects** (one doc per epic), the prefix scheme is mandatory — one prefix per epic doc, agreed up front in Phase 1 (see *Multi-document projects* in the Output File section). Do not start Phase 2 until the prefix list is recorded in `session_memory-specifications-<topic>.md`.

#### Step 2: Derive specification items from the vision
...
- For each constraint → what specification items does it imply?

Validate as you derive: "I see [scenario] implies the system needs to [capability]. Is that right?"

Ask explicitly: "Are there specification items that aren't implied by the scenarios but are still needed? What about administrative or operational specification items — monitoring, backup, user management?"

**Backlink scenarios to specification items.** After specification items are drafted, update each Phase 1 scenario in §1.3 to link the specifications it illustrates. Add *(see [SPEC-004](#spec-004-title), [SPEC-012](#spec-012-title))* at natural sentence breaks within the scenario — or at the end of the scenario paragraph where several specification items apply together. These are markdown heading anchors and must match the actual heading IDs in the document.

#### Step 3: Write individual specification items

Each specification item follows the template:
...
**Dependencies:** None (or markdown links to other specification item headings)
...
Every specification ID referenced anywhere in the document must be a **markdown link** to that specification item's section heading.

#### Step 4: Map the dependency graph

After all specifications are drafted:
- Check for circular dependencies (these reveal a specification items problem — resolve them)
- Identify foundational specification items that many others depend on
- Verify the ordering is implementable: can a team start with no-dependency specification items and work forward?

#### Step 5: Check completeness

Before presenting to the human, verify:
- [ ] Every actor from Phase 1 has at least one specification item addressing their goals
- [ ] Every scenario from Phase 1 is covered by one or more specification items
- [ ] Every constraint has corresponding specification items
- [ ] Error cases and edge cases are covered
- [ ] Non-functional specifications are included where they affect behavior (audit logging, access control, data retention, etc.)
- [ ] No specification item violates the out-of-scope boundaries from Phase 1

Then ask: "I've identified [N] specification items across [M] functional areas. Before I present them, is there anything you know is needed that we haven't discussed? What about [specific uncovered area]?"

#### Step 6: Internal consistency review

Before the Phase 2 checkpoint, scan all specifications for internal consistency. **This step is not optional.** Specification Items that will be used to drive AI must be free of contradictions — ambiguities a human resolves from context will cause AI to produce unpredictable results.

Check for:

- **Direct contradictions** — SPEC-A asserts something that SPEC-B explicitly denies. Both cannot be true.
- **Acceptance criteria conflicts** — two specification items' Given/When/Then clauses produce incompatible outcomes for the same actor/scenario combination.
- **Scope boundary violations** — specification items that describe behaviour the scope section marks as out of scope.
- **Ambiguous shared terms** — the same word used with different meanings in different specification items (e.g. "user" meaning customer in one place, admin in another). These are not contradictions but will cause AI misreading.
- **Dependency integrity gaps** — SPEC-A depends on SPEC-B, but SPEC-B's acceptance criteria do not actually satisfy what SPEC-A needs from it.
- **Implicit conflicts** — two specification items that don't directly contradict but together create an impossible or undesirable state.

For each finding, produce a labelled finding block:

```
CONFLICT: SPEC-004 vs SPEC-011
SPEC-004: users may delete their account at any time.
SPEC-011: all orders must retain a customer reference for 7 years (compliance).
Tension: hard-delete conflicts with the data-retention obligation.
```

For each finding, offer:
- **Resolve inline** — user provides a clarification; AI updates the affected specification items immediately.
- **Add to open questions log** as a new `Q-NNN` entry for brainstorming before the checkpoint.
- **Accept as known trade-off** — explicitly document the tension and the chosen resolution in both specification items.

**Do not present a specification set with unresolved contradictions.** A document with known conflicts is not a useful artefact — not for humans, and especially not for AI.

#### Phase 2 Checkpoint

**STOP.** Present the full specification items list, then the consistency review findings (if any). Say:

> "Here are the concrete specification items derived from the vision.
>
> Consistency review: [N conflicts found / no conflicts found].
> [List each finding with proposed resolution if applicable.]
>
> Please review:
> - Are all specifications correct and clearly stated?
...
- "Are there external deadlines certain specification items must hit?"
- "What's the minimum set that delivers usable value — the MVP?"

Look for specification items that share a stakeholder-visible outcome or belong to a coherent business capability. Do **not** group by component, dependency depth, or technical layering — those are roadmap concerns.
...
- Specification Items listed as markdown links to their definitions
- Optional: priority label (`MVP` / `P1` / `P2` / `Later`)

The first group should be the MVP — the smallest set of specification items that delivers usable value. Identify it explicitly.

#### Step 3: Validate

- [ ] Every specification item appears in exactly one group
- [ ] No specification item's dependency sits in a later group (use the Phase 2 dependency graph to check)
- [ ] The MVP group is genuinely minimal

#### Step 4: Add stakeholder guidance

- **MVP rationale:** why these specification items form the minimum viable set
...
After approval, write the file per [Output File](#output-file) rules. Then offer to write Episodes and to run a Stability Audit before offering to delete `session_memory-specifications.md`.

---

## As-You-Go Health Checks

Specifications documents are built incrementally — inconsistencies accumulate naturally. Trigger a **health check** whenever a meaningful volume of material has landed: approximately every 8–10 specification items discussed, at every phase checkpoint, or after any non-trivial revision. The user can also ask for one at any time.
...
|--------|-------------|--------|
| **Mechanical** | Terminology inconsistencies, broken cross-references, a term used two ways | Fix silently and note: *"Corrected: 'user' was ambiguous in SPEC-003 and SPEC-007 — unified to 'customer'."* |
| **Potential contradiction** | Two specification items that could conflict but might be reconcilable with a clarification | Surface with a proposed resolution and wait for confirmation before editing. |
| **Genuine gap or ambiguity** | An unclear specification item, a missing constraint, an implicit assumption needing a decision | Add to the open questions log as `Q-NNN` and interview the user. |
...
## Specification Item Episodes

Episodes are narrative walkthroughs of the system from a user's perspective — stories that illustrate how specification items combine in real use. Write them when the specifications alone feel abstract or when stakeholders need to see the system "in action" to validate completeness.

When to write, voice/tone guidance, SPEC-NNN reference conventions, and the output file format: [episodes-guide.md](./references/episodes-guide.md).

## Stability Audit (Final Convergence Review)

When Phase 3 is approved and episodes are written (or the user signals the specifications are approaching final form), offer a stability audit:
...
Each round scans the complete specifications document:
- Vision and scope for internal coherence
- Specification Item descriptions against their acceptance criteria
- Cross-specification item dependencies for integrity
...
After each round, present a structured findings list: finding ID, affected specification item(s), issue type, severity (`blocking` / `minor` / `cosmetic`), and proposed resolution. Handle findings the same three-bucket way as the as-you-go health checks.
...
- Capture the *why* behind features. Understanding motivation helps identify missing specification items later.

**During specification items:**
- Write specification items that are testable. If you can't imagine how to verify it, it's too vague.
- Don't sneak implementation into specification items. Describe behavior, not mechanism.
- Look for implicit specification items: "real-time updates" implies WebSocket support, notification systems, etc.
...
**"This specification item is really an implementation choice"**
...
- Fix: Ask "Why do you want [technical choice]?" — the answer reveals the real specification item.

**"The specification item is too vague"**
- Bad: "The system shall be fast."
...
- Fix: Look for specification items that are actually two combined. Split them. The shared part becomes a foundational specification item.

## Anti-patterns

- Proceeding past a phase checkpoint without explicit human approval.
- Writing implementation details into specification items ("shall use PostgreSQL").
...
- **Skipping health checks** when many specification items have accumulated. Contradictions don't self-report.

## Sibling Skills

This skill produces a specifications document that feeds directly into:
...
1. **Restate** the question in full, with context: which specification items are affected, what the tension is, and why the decision matters.
...
4. **When the user decides**: mark the question `resolved` in the open questions log, add a one-line resolution note, and update every affected specification item.
5. **When the user defers**: mark it `deferred`, note the reason, and annotate each affected specification item with the open question number so the gap is visible in the output:
