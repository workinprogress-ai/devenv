...
- FOUND: preserve the repro test before any restore, then continue with `/devenv-hunt` diagnose + fix modes using this report. The repro is the starting artifact.
...
- resolution: kill now | plan (/devenv-plan) | handoff (/devenv-pair | /devenv-delegate) | self-fix | undecided
