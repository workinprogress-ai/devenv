...
- **Explicit request only, with one narrow exception** — same gate as distillation, same rationale (#26: proactive meta-offers are noise). The exception: `/devenv-design` may surface a pattern-shaped discovery once, in flight, as a one-line offer when the generalization becomes apparent mid-discussion — capture still happens only if the user takes the offer. No wrap-up capture menus anywhere.
- **Only `/devenv-design` writes to the engineering repo, and only via PR** — it prepares any change (new entries and edits to existing entries, from proposal issues or session discoveries) in the repo clone on a branch, shows the diff for approval, and surfaces it via `pr-create-for-merge`. The user's merge is the ratification; GitHub's permission model is the enforcement. No other skill touches that repo, and no skill merges.
